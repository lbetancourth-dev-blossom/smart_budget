...
if (
    not _OLB_CHECK.exists()
    and (ROOT.parent.parent / "data" / "olb" / "dev" / "silver").exists()
):
...
def build_sub_transactions(olb: Path) -> pd.DataFrame:
    ...
    if "idolbtransactioninfo" in df.columns and not oti.empty:
        df = df.merge(
            oti[["id", "idolbtransactioncategory", "note"]].rename(
                columns={"id": "_oti_id"}
            ),
            left_on="idolbtransactioninfo",
            right_on="_oti_id",
            how="left",
        )
    else:
    ...
    if "idolbtransactioncategory" in df.columns and not otc.empty:
        df = df.merge(
            otc[["id", "name"]].rename(
                columns={"id": "_otc_id", "name": "defaultcategory"}
            ),
            left_on="idolbtransactioncategory",
            right_on="_otc_id",
            how="left",
        )
    else:
    ...
    sub_income_exp = sub_amount_raw.apply(
    ...
    result = pd.DataFrame(
        {
            ...
            "idOLBTransactionInfo": df.get(
                "idolbtransactioninfo", pd.Series([None] * len(df))
            ).astype(str),
...
def build_loan_transactions(olb: Path) -> pd.DataFrame:
    ...
    # Se normaliza a positivo para alinear con la convención de EXT (Plaid/Finicity).
    loan_amount_raw = (
        pd.to_numeric(df[amount_col], errors="coerce") if amount_col else None
    )
    loan_income_exp = (
        loan_amount_raw.apply(
            lambda a: "expenditure" if float(a or 0) < 0 else "income"
        )
...
def build_external_transactions(dough: Path) -> pd.DataFrame:
    ...
    date_col = (
        ...
        else ext.get("effectivedate", ext.get("createdat"))
    )
    ...
    result = pd.DataFrame(
        {
            ...
            "idAccount": "EXT"
            + ext.get("idaccount", pd.Series([None] * len(ext))).astype(str),
            "idSubAccount": None,
            "idCategory": (
                idcat_aligned
                if default_cat_path.exists()
                else pd.Series([None] * len(ext))
            ),
            "amount": amount_abs,
            ...
            "description": (
                ext["description"] if "description" in ext.columns else ext.get("name")
            ),
            "balance": None,
    ...
    print(
        f"  ✅ EXT transactions: {len(result):,} ({exp_count} expenditure, {len(result)-exp_count} income)"
    )
    return result


def build_from_db(
    host: str, dbname: str, user: str, password: str, port: int = 5432
) -> pd.DataFrame:
    """
...
def save_outputs(fact: pd.DataFrame):
    ...
    fact["checknumber"] = pd.to_numeric(fact["checknumber"], errors="coerce").astype(
        "Int64"
    )
    ...
    fact["description"] = (
        fact["description"].astype(str).str.replace('"', "'", regex=False).str.strip()
    )
    ...
    fact.to_csv(out, **kw)
    print(
        f"  ✅ fact_transactions.csv          → {len(fact):,} filas, {len(fact.columns)} cols"
    )
    ...
    print(
        f"  ✅ fact_transactions_expenditure.csv → {len(exp):,} filas (solo gastos, apto Excel)"
    )
    ...
    print(
        f"  ✅ fact_transactions_sample.csv   → {len(sample):,} filas (muestra aleatoria)"
    )
...
def main(
    ...
    if source == "db":
        ...
        if not user or not pwd:
            print(
                "❌ Credenciales DB requeridas. Usa --db-user / --db-pass o env vars DB_USER / DB_PASS"
            )
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--db-host", default=None, help="DB host (default: dev cluster)"
    )
    parser.add_argument(
        "--db-name",
        default=None,
        help="DB name (default: blossom-dough-consolidated-dev)",
    )
    parser.add_argument("--db-user", default=None, help="DB user (o env var DB_USER)")
    parser.add_argument(
        "--db-pass", default=None, help="DB password (o env var DB_PASS)"
    )
