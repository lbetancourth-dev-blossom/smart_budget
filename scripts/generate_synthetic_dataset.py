...
def random_amount(
    category: str, rng: np.random.Generator, seasonal_mult: float = 1.0
) -> float:
...
def build_synthetic_members(
    ...
    for i in range(1, n_members + 1):
        ...
        if member_category_map and member_id in member_category_map:
            ...
            member_cats = random.sample(
                EXPENSE_CATEGORIES, min(n_cats, len(EXPENSE_CATEGORIES))
            )
...
def enrich_existing_members(
    ...
    for member_id in members:
        ...
        existing_cats = set(
            df_existing[df_existing["idaccount"] == member_id][
                "defaultcategory"
            ].unique()
        )
        ...
        for cat in new_cats:
            for period in periods:
                ...
                rows.append(
                    {
                        "idclient": df_existing[df_existing["idaccount"] == member_id][
                            "idclient"
                        ].iloc[0],
                        "idcompany": df_existing[df_existing["idaccount"] == member_id][
                            "idcompany"
                        ].iloc[0],
                        "idaccount": member_id,
                        "idcategory": CATEGORY_ID_MAP.get(cat, "99"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Genera dataset sintético Smart Budget"
    )
    parser.add_argument(
        "--months", type=int, default=6, help="Últimos N meses a incluir (default: 6)"
    )
    parser.add_argument(
        "--extend-months",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Miembros sintéticos nuevos (default: 8)",
    )
    parser.add_argument(
        "--extra-cats",
        type=int,
        default=4,
        help="Categorías extra por miembro existente",
    )
    parser.add_argument(
        "--input",
        default=None,
    ...
    input_path = (
        Path(args.input)
        if args.input
        else root / "data" / "dough" / "smart_budget_prep.csv"
    )
    output_path = (
        Path(args.output)
        if args.output
        else root / "data" / "dough" / "smart_budget_synthetic.csv"
    )
    ...
    print(
        f"📅 Periodos base ({args.months} meses): {last_periods[0]} → {last_periods[-1]}"
    )
    ...
    df_filtered["defaultcategory"] = (
        df_filtered["defaultcategory"].str.title().str.strip()
    )
    df_filtered = df_filtered[df_filtered["defaultcategory"].isin(EXPENSE_CATEGORIES)]
    print(
        f"✅ Miembros existentes: {df_filtered['idaccount'].nunique()} "
        f"| Categorías: {df_filtered['defaultcategory'].nunique()}"
    )

    # Enriquecer miembros existentes con categorías adicionales (periodos base)
    print(
        f"➕ Agregando hasta {args.extra_cats} categorías nuevas por miembro existente..."
    )
    df_enriched = enrich_existing_members(
        df_filtered, last_periods, rng, args.extra_cats
    )
    ...
    if args.extend_months > 0:
        ...
        print(
            f"⏪ Generando {args.extend_months} meses previos: {prior_periods[0]} → {prior_periods[-1]}"
        )
        df_prior_existing = extend_existing_members_back(
            df_filtered, prior_periods, rng
        )
    ...
    df_final = pd.concat(
        [df_prior, df_filtered, df_enriched, df_new], ignore_index=True
    )

    # Ordenar
    df_final = df_final.sort_values(
        ["idaccount", "defaultcategory", "period_yyyymm"]
    ...
    print(
        f"   Miembros        : {total_members} ({df_filtered['idaccount'].nunique()} existentes + {args.new_members} nuevos)"
    )
    print(f"   Categorías      : {total_cats}")
    print(
        f"   Meses           : {all_final_periods[0]} → {all_final_periods[-1]} ({len(all_final_periods)} meses)"
    )
    ...
    summary = (
        ...
        .agg(
            ...
            median_amount=(
                "monthly_total",
                lambda x: round(x[x > 0].median(), 2) if (x > 0).any() else 0,
            ),
        )
        .reset_index()
    )
