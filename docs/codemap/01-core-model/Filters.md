...
Aplica las 5 reglas de filtrado **no negociables** sobre `fact_transactions` para garantizar que solo ingresen al modelo transacciones válidas, completadas y presupuestables.

## Las 5 reglas (en orden de aplicación)
...
flowchart TD
    ...
    R4["Regla 4: OLB status\n(SUB/LOAN: excluir PENDING y HOLD)"]
    R5["Regla 5: EXT status\n(EXT: solo POSTED)"]
...
### Regla 2 — Solo gastos (A3)

```python
df = df[df["incomeexpenditure"] == "expenditure"]
```

Excluye ingresos. Smart Budget solo presupuesta gastos.
...
Para transacciones OLB: excluir si `status IN ('PENDING', 'HOLD')`. Status `NULL` y `CLEARED` pasan.

### Regla 5 — EXT (Plaid/Finicity) status (A6)
...
Los prefijos que no son `SUB`, `LOAN` ni `EXT` **pasan sin filtro de status**. Diseño intencional para evitar data loss silencioso. Se loguea una advertencia si se encuentran.
