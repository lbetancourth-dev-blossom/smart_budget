...
def _load_golden() -> pd.DataFrame:
    return pd.read_csv(
        _FIXTURES_DIR / "golden_set.csv", dtype=str, keep_default_na=False
    )
...
def test_TC_T6_4_golden_set_matches_wma_output():
    ...
    golden = _load_golden()
    golden["suggested_amount"] = pd.to_numeric(
        golden["suggested_amount"], errors="coerce"
    )
    ...
    buckets = golden[
        ["idmember", "category_id", "defaultcategory", "suggested_amount"]
    ].drop_duplicates(subset=["idmember", "category_id", "defaultcategory"])
