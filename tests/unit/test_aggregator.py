...
def test_prepare_smart_budget_data_end_to_end():
    ...
    filtered = filtered.rename(
        columns={"idcategory": "category_id", "defaultcategory": "category_name"}
    )
    ...
    if len(result) > 0:
        counts = (
            result[result["monthly_total"] > 0]
            .groupby(["idmember", "category_name"])["period_yyyymm"]
            .nunique()
        )
        assert (counts >= 3).all()
...
def test_TC_T3_4_apply_gating_uses_idmember_grain():
    ...
    unique_buckets = result[
        ["idclient", "idcompany", "idmember", "category_name"]
    ].drop_duplicates()
    assert (
...
def test_TC_T3_5_apply_gating_prevents_cross_company_mixing():
    ...
    assert (
        "CO2" not in result["idcompany"].values
