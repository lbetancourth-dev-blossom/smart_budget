...
class TestTC_T7_1_NoCrossMemberLeak:
    ...
    def test_total_suggested_is_member_scoped(self):
        ...
        """
        # Arrange
        rows = _member_df(10, 1, 1, "CAT1", "Groceries", [100.0] * 6) + _member_df(
            20, 1, 1, "CAT1", "Groceries", [500.0] * 6
        )
        df = _make_monthly_df(rows)

        # Act
        results = compute_budget_suggestions(
            df, method="wma", treatment="A", reference_date="2026-03-01"
        )
...
class TestTC_T7_2_CrossCompanyCollision:
    ...
    def test_raises_on_cross_company_idmember(self):
        ...
        rows = _member_df(
            10, 1, 1, "CAT1", "Groceries", [100.0] * 6
        ) + _member_df(  # company=1
            10, 1, 2, "CAT1", "Groceries", [200.0] * 6
        )  # company=2 ← collision
        df = _make_monthly_df(rows)
