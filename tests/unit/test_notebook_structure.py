...
NOTEBOOK_PATH = (
    ...
    / "notebooks"
    / "smart_budget_sagemaker_endpoint.ipynb"
)
...
def test_notebook_has_no_data_csv_packaging():
    ...
    for src in sources:
        ...
        assert (
            "smart_budget_data.csv" not in src
        ), f"Found 'smart_budget_data.csv' in cell: {src[:100]!r}"
...
def test_notebook_sklearn_model_passes_env():
    ...
    assert (
        "ATHENA_S3_STAGING_DIR" in sklearn_src
    ), "env= does not reference ATHENA_S3_STAGING_DIR"
...
def test_notebook_documents_iam_requirements():
    ...
    for src in markdown_cells:
        if (
            "athena:StartQueryExecution".lower() in src.lower()
            and "execution role" in src.lower()
        ):
            return
