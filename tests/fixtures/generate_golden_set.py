...
def build_synthetic_data() -> pd.DataFrame:
    ...
    for idcat, defcat, totals in member10_data:
        for period, total in zip(PERIODS_6, totals):
            rows.append({
                ...
                "idmember": "10", "idaccount": "EXT2",
                "idcategory": idcat, "defaultcategory": defcat,
                "period_yyyymm": period, "monthly_total": total,
            })
    ...
    member20_data_sub8406 = [
        ("1", "Auto & Transport",    [30.0,  32.0,  35.0,  31.0,  33.0,  34.0]),  # Will be summed with EXT22
...
def generate_golden_set(output_path: pathlib.Path) -> None:
    ...
    print(f"  After gating: {len(prepared)} rows, {prepared['idmember'].nunique()} members")
    ...
    for r in results:
        ...
        sugg_map[key] = {
            ...
            "months_with_positive_spend": r["basis"]["months_with_positive_spend"] if r["basis"] else None,
        }
    ...
    for _, row in prepared.iterrows():
        key = (str(row["idmember"]), str(row["idcategory"]), str(row["defaultcategory"]))
    ...
    assert golden_df["period_yyyymm"].nunique() == 6, (
